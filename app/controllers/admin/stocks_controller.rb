class Admin::StocksController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @admin_stock.update(admin_stock_params)
        format.html { redirect_to @admin_stock, notice: "Stock was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_stocks_path, notice: "Stock was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_admin_stock
      @admin_stock = Admin::Stock.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def admin_stock_params
      params.expect(admin_stock: [ :product_id, :amount, :size ])
    end
end
